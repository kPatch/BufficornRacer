...
namespace TS.Generics
{
    public class InfoInputs : MonoBehaviour
    {
        ...
        public void checkInputs()
        {
            ...
            for (var j = 0; j < ListOfInputsForEachPlayer.Count; j++)
            {
                for (var i = 0; i < ListOfInputsForEachPlayer[j].listOfButtons.Count; i++)
                {
                    ...
                    if (ControlFreak2.CF2Input.GetKey(nKeycode))
                        value = 1;

                    if (Mathf.Abs(ControlFreak2.CF2Input.GetAxisRaw(ListOfInputsForEachPlayer[j].listOfButtons[i]._AxisName)) > .2f)
                        value = ControlFreak2.CF2Input.GetAxisRaw(ListOfInputsForEachPlayer[j].listOfButtons[i]._AxisName);
        ...
        public bool checkSpecificInput(int playerID, int buttonID)
        {
            ...
            //-> Check Gamepad
            if (value <= .2f && ControlFreak2.CF2Input.GetAxisRaw(ListOfInputsForEachPlayer[playerID].listOfButtons[buttonID]._AxisName) != 0)
            {
                //Debug.Log("_AxisName: " + ListOfInputsForEachPlayer[playerID].listOfButtons[buttonID]._AxisName);
                value = ControlFreak2.CF2Input.GetAxisRaw(ListOfInputsForEachPlayer[playerID].listOfButtons[buttonID]._AxisName);
